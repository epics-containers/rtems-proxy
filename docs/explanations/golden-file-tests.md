...
"last reviewed" result, and the test re-runs the pipeline and compares.

For the workflow itself, see
[](../how-to/update-test-baselines.md).
...
- **`ibek-support` and `ibek-support-dls` are git submodules** at the repo root,
  ...
  installed `builder2ibek` does **not** read them). They track `main`, so
  Renovate's `git-submodules` manager opens a PR each time `main` advances, runs
  these tests on the bump, and gives you a reviewable diff.
- **`builder2ibek` is pinned in a non-default `ci` dependency group** (see
  `[dependency-groups]` in `pyproject.toml`), rather than being run via
  `uvx builder2ibek`. Pinning means a new release arrives as a Renovate PR that
  *runs these tests* — so an incompatible change is caught and **attributed to a
  specific version bump**, instead of silently turning CI red with nothing to
  point at.
...
hybrid tests are therefore designed to run on a **DLS-internal runner or
...
implies redeploying *every* IOC), the diff size makes the cost visible and
nudges you toward making changes opt-in or parameterised when they can be,
rather than globally flipping behaviour.
